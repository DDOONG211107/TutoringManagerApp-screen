...
export default MatchingHome = ({ navigation, route }) => {
  ...
  useEffect(() => {
    ...
    teacherInfoList_.map(async (teacherItem) => {
      ...
      const docSnap = await getDoc(
        doc(
          ...
          teacherItem?.uid,
          'matching',
          teacherItem?.teacherCurrentMatchingId
        )
      );
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            {subject
              ? `${subject} / 월 ${money / 10000}만원`
              : '현재 매칭 정보가 없습니다'}
          </Text>

          <Text style={{ fontSize: 18, color: style.colorList.navy }}>
            수업 방식 : {getTeachingTypeString(teachingType)}
          </Text>
        </View>
        <View
          style={{
      ...
      >
        ...
        >
          ...
          >
            {/* <Ionicons name='settings' size={30} color='black' /> */}
            <Text style={{ color: style.colorList.white }}>
              나의 추천 비율 불러오기
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
