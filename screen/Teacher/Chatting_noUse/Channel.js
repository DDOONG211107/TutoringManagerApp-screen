  ...
  return (
    ...
    >
      <MaterialIcons
        name='send'
        size={24}
        color={props.text ? style.colorList.blue : style.colorList.grey_1}
      ></MaterialIcons>
    </Send>
  );
};
...
const Channel = ({ navigation, route: { params } }) => {
  ...
  useLayoutEffect(() => {
    navigation.setOptions({ headerTitle: params.guestName || 'Channel' });
  }, []);
  ...
  const _handleMessageSend = async (messageList) => {
    ...
    await createMessage({ channelId: params.id, message: newMessage })
      .then(async () => {
        await getDocs(
          query(collection(db, 'users'), where('id', '==', params.inviteUser))
        ).then(async (users) => {
          await pushNotificationsToPerson(
            auth.currentUser.displayName,
            users.docs[0].data().uid,
            '새로운 채팅',
            newMessage.text
          );
        });
